...
void draw(){
  ...
  for(float r = 0; r < 500; r += 25){
    ...
    //stroke(220 - (20 * log((1 + (Math.abs((mic.left.level() + mic.right.level()) / 2)) / (1 + (20 * log(1 + (Math.abs((mic.left.level() + mic.right.level()) / 2))))))) * 207), 88 + (Math.abs((mic.left.level() + mic.right.level()) / 2) * 63), 141 + (Math.abs((mic.left.level() + mic.right.level()) / 2) * 89)); // eddie
